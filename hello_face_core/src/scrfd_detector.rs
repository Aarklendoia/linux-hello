...
pub struct ScrfdDetector {
    ...
    nms_threshold: f32,
    /// Anchor centers for each of the 3 strides (8/16/32), precomputed once
    /// here rather than regenerated by `generate_anchor_centers` on every
    /// `detect()` call — they only depend on `input_size`, which is fixed
    /// for this detector's whole lifetime (every registration frame during
    /// enrollment, and every scored frame in verify()'s per-frame loop, was
    /// otherwise redoing the same ~10,800 `(cx, cy)` pushes from scratch).
    anchors_per_stride: [Vec<(f32, f32)>; 3],
}
...
impl ScrfdDetector {
    pub fn load(model_path: &Path) -> Result<Self, FaceError> {
        ...
        let input_size = 640;
        let anchors_per_stride =
            [8u32, 16, 32].map(|stride| Self::generate_anchor_centers(stride, input_size));
    ...
    fn detect(
        ...
        let regions_raw: Vec<FaceRegion> = {
            ...
            for (s_idx, &stride) in strides.iter().enumerate() {
                ...
                let anchor_centers = &self.anchors_per_stride[s_idx];
